...
def get_connection():
    conn = sqlite3.connect("C:/Users/USER/PersonalTaskTracker/data/tasks.db")
...
def add_task(description: str, details:str,  priority:str, due_date: str = None):
    with get_connection() as conn:
        conn.execute('''
            INSERT INTO tasks (description, details, due_date, priority)
            VALUES (?, ?, ?, ?);
        ''', (description, details, due_date, priority))
        conn.commit()
        
def get_tasks(archived: bool = False): 
    with get_connection() as conn:
        rows = conn.execute('SELECT * FROM tasks WHERE archived = ? ORDER BY id; ', 
...
def sort_tasks_and_or_priority(due_date: 0, priority: 0):
    query = 'SELECT * FROM tasks WHERE archived = 0'
    order_clauses = []

    if due_date:
        order_clauses.append('due_date')
    if priority:
        # Custom ordering for priority: high > mid > low
        order_clauses.append(
            "CASE priority WHEN 'high' THEN 1 WHEN 'mid' THEN 2 WHEN 'low' THEN 3 END"
        )

    if not order_clauses:
        raise Exception("At least one sorting criteria must be specified.")
